...
class NAFNetGenerator(nn.Module):
    """Create a NAFNet-based generator"""

    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=LayerNorm, use_dropout=False, width=32, middle_blk=4):
        """Construct a Naf generator
        Parameters:
            input_nc (int)  -- the number of channels in input images
            output_nc (int) -- the number of channels in output images
            num_downs (int) -- the number of downsamplings in UNet. For example, # if |num_downs| == 7,
                                image of size 128x128 will become of size 1x1 # at the bottleneck
            ngf (int)       -- the number of filters in the last conv layer
            norm_layer      -- normalization layer - forced to be norm_layer
        ...
        self.intro = nn.Conv2d(in_channels=input_nc, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)
        self.ending = nn.Conv2d(in_channels=width, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)
        ...
        self.middle_blks = \
            nn.Sequential(
                *[NAFBlock(chan) for _ in range(middle_blk)]
            )
